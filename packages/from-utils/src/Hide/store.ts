...
export class HideStore {
  ...
  //保存组件
  componentLists: HideFieldEntity[] = [];
  // 初始化
  private init = (entery: HideFieldEntity) => {
  ...
  // 挂载之后 组件保存
  private register = (entery: HideFieldEntity) => {
...
/** 状态管理 */
export const useHideFormItem = (form?: HideGetStoreProps) => {
  const formRef = React.useRef<HideGetStoreProps>();
  if (!formRef.current) {
    if (form) {
